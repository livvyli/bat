...
T = array2table(NaN(length(rownames),length(columnnames)), 'VariableNames', columnnames_cell, 'RowNames', rownames_cell);
...
for idx_t = 1:length(t)
    ...
    for idx_tau = 1:length(tau)
        ...
        % Calculate row index based on t(idx_t)
        row_idx = find(rownames == t(idx_t));
        
        % Calculate column index based on tau(idx_tau)
        col_idx = find(columnnames == tau(idx_tau));
        if isKey(vx_a_lookup, t(idx_t)) && isKey(vx_b_lookup, t(idx_t) + tau(idx_tau))
            ...
            T{rownames_cell(row_idx), columnnames_cell(col_idx)} = v_ab;
        else
